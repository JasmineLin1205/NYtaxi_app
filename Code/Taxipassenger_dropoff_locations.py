...
def show_taxipassenger_dropoff():
    ...
    if selected_month in ["1月", "2月"]:
        columns_for_clustering = ["dropoff_latitude", "dropoff_longitude"]
    else:
        columns_for_clustering = [" dropoff_latitude", " dropoff_longitude"]
    ...
    # 添加一个下拉菜单以选择区域
    selected_region = st.selectbox("選擇我想看的區域", list(region_dbscan_params.keys()))
    ...
    if not sample_data_copy.empty:
        for _, row in sample_data_copy.iterrows():
            if "dropoff_latitude" in data.columns:
                popup_content = f"緯度: {row['dropoff_latitude']}經度: {row['dropoff_longitude']}"
                folium.Marker(
